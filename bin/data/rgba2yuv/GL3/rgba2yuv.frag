...
void main()
{
	vec2 texSize = vec2(textureSize(rgbatex, 0));
	vec2 texNormScale = vec(1) / (texSize - vec(1)); // [0,W-1] -> [0,1]

	// RGBA texture is twice as wide as the source YUV texture
	vec4 rgba0 = texture(rgbatex, texNormScale* vec2(texCoord.x*2, texCoord.y));
	vec4 rgba1 = texture(rgbatex, texNormScale* vec2(texCoord.x*2+1, texCoord.y));
